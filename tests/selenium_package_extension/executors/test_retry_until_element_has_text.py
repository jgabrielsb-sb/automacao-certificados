...
from automacao_certificados.selenium_package_extension.executors import RetryUntilElementHasText
...
class TestRetryUntilElementHasText:
    """
    Test class for the RetryUntilElementHasText executor.
    """
    ...
    def test_if_returns_true_if_the_element_has_the_desired_text(
        ...
        executor = RetryUntilElementHasText(
            action=action_class(
                driver=MagicMock(spec=WebDriver),
            ),
    ...
    def test_if_returns_false_if_the_element_does_not_have_the_desired_text(
        self,
        action_class: type[BaseAction],
    ):
        """
