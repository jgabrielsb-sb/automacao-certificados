...
from automacao_certificados.selenium_automations.adapters.selenium.exceptions import CNPJNotFoundException
...
class TestRunDocumentDelmiroDownloader:
    def test_if_raises_value_error_if_cnpj_is_not_a_number(self, consulta_page, download_page):
        with pytest.raises(ValueError):
            DocumentDelmiroDownloader(
                consulta_page=consulta_page,
                download_page=download_page,
            ).run(input=DocumentDownloaderInput(cnpj="not a number"))

    def test_if_raises_value_error_if_cnpj_has_less_than_14_digits(self, consulta_page, download_page):
    ...
    @pytest.mark.selenium_workflow_tests
    def test_if_raises_cnpj_not_found_exception_if_cnpj_is_not_found(self, consulta_page, download_page):
        with pytest.raises(DocumentDownloaderException) as e:
            DocumentDelmiroDownloader(
                consulta_page=consulta_page,
                download_page=download_page,
            ).run(input=DocumentDownloaderInput(cnpj="12345678901234"))

        print(e.value)
        assert "CNPJ not found error: the CNPJ 12345678901234 is not found." in str(e.value)
