...
class RetryUntilElementContainsText(BaseExecutor):
    """
    Executor that retries an action until an element contains a specific text.
    """
    def __init__(
        self,
        action: BaseAction,
        ...
        """
        Initializes the retry until element contains text executor.
        Args:
            action: The action to be executed and retried.
            web_element: The web element to check the text of.
            desired_text: The text to check if the element contains.
    ...
    def _is_condition_to_stop_met(self) -> bool:
        if self.desired_text in self.web_element.text:
            return True
