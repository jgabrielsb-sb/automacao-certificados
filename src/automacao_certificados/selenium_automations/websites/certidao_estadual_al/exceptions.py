...
class InvalidCaptchaException(ConsultaPageException):
    ...
    def __init__(
        ...
        """
        Args:
            captcha_value: The value of the captcha.
        """
        if not isinstance(captcha_value, str):
            raise ValueError("captcha_value must be a string")
        
        super().__init__(
            f"Invalid captcha error: the captcha value {captcha_value} is not correct."
        )
